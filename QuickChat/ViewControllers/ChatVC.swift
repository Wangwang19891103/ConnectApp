...
class ChatVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate,  UINavigationControllerDelegate, UIImagePickerControllerDelegate, CLLocationManagerDelegate {
    ...
    func customization() {
        ...
        let icon = UIImage.init(named: "back1")?.withRenderingMode(.alwaysOriginal)
        let backButton = UIBarButtonItem.init(image: icon!, style: .plain, target: self, action: #selector(self.dismissSelf))
        self.navigationItem.leftBarButtonItem = backButton
        let backicon = UIImage.init(named: "home4")?.withRenderingMode(.alwaysOriginal)
        let rightButton = UIBarButtonItem.init(image: backicon!, style: .plain, target: self, action: #selector(ChatVC.goConversationVC))
    ...
    func goConversationVC(){
        
        if #available(iOS 10.0, *) {
            let mainVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "Conversations") as! ConversationsVC
    ...
    @IBAction func sendMessage(_ sender: Any) {
        if let text = self.inputTextField.text {
            if text.characters.count > 0 {
                ...
                Database.database().reference().child("users").child(self.currentUser!.id).child("playerID").observeSingleEvent(of: .value, with: { (snapshot) in
                    
                    if let data = snapshot.value as? [String: Any] {
                        
                        print("Message receiver playerID is : \(data["playerID"] as! String)")
                        
                        var playerID = [String]()
                        playerID.append(data["playerID"] as! String)
                        
                        OneSignal.promptForPushNotifications(userResponse: { accepted in
                            print("User accepted notifications: \(accepted)")
                        })
                        
                        let message = "Someone sent you new messages : \(messageText)"
                        
                        OneSignal.postNotification(["contents": ["en": message], "include_player_ids": playerID], onSuccess: { (dict) in
