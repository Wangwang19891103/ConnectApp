...
class NavVC: UINavigationController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate, UINavigationControllerDelegate {
    ...
    var homeVC: ConversationsVC!
    var profileVC: ProfileVC!
    var mapVC: MapViewController!
    var questionVC: QuestionVC!
    var settingVC: FilterVC!
    var helpVC: HelpViewController!
    
    
    func homeViewController() -> ConversationsVC {
        homeVC = self.storyboard?.instantiateViewController(withIdentifier: "Conversations") as! ConversationsVC
        return homeVC
    }
    
    func showHomeViewController(){
        self.setViewControllers([self.homeViewController()], animated: true)
    }
    
    func profileViewController() -> ProfileVC{
        profileVC = self.storyboard?.instantiateViewController(withIdentifier: "ProfilePage") as! ProfileVC
        return profileVC
    }
    
    func showProfileViewController(){
        self.setViewControllers([self.profileViewController()], animated: true)
    }
    ...
    func showMapViewController(){
        self.setViewControllers([self.mapViewController()], animated: true)
    }
    
    func questionViewController() -> QuestionVC{
        questionVC = self.storyboard?.instantiateViewController(withIdentifier: "questionVC") as! QuestionVC
        return questionVC
    }

    func showQuestionViewController(){
        self.setViewControllers([self.questionViewController()], animated: true)
    }
    
    func filterSettingViewController() -> FilterVC{
        settingVC = self.storyboard?.instantiateViewController(withIdentifier: "filter") as! FilterVC
        return settingVC
    }
    
    func showFilterSettingViewController(){
        self.setViewControllers([self.filterSettingViewController()], animated: true)
    }
    
    func helpViewController() -> HelpViewController{
        helpVC = self.storyboard?.instantiateViewController(withIdentifier: "help") as! HelpViewController
        return helpVC
    }
    
    func showHelpViewController(){
        self.setViewControllers([self.helpViewController()], animated: true)
    }
    ...
    @IBAction func setQuestionnarie(_ sender: Any) {
        
        self.performSegue(withIdentifier: "questionnariesegue", sender: nil)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        }else if segue.identifier == "questionnariesegue"{
            
           _ = segue.destination as! QuestionVC
            self.dismissExtraViews()
        }
